...
由于 Scope Hoisting 需要分析出模块之间的依赖关系，因此源码必须采用 ES6 模块化语句，不然它将无法生效。 原因和[4-10 使用 TreeShaking](4-10使用TreeShaking.html) 中介绍的类似。
...
同时，考虑到 Scope Hoisting 依赖源码需采用 ES6 模块化语法，还需要配置 `mainFields`。 原因在 [4-10 使用 TreeShaking](4-10使用TreeShaking.html) 中提到过：因为大部分 Npm 中的第三方库采用了 CommonJS 语法，但部分库会同时提供 ES6 模块化的代码，为了充分发挥 Scope Hoisting 的作用，需要增加以下配置：
...
> 本实例<a href="../zip/4-14开启ScopeHoisting.zip" target="_blank">提供项目完整代码</a>