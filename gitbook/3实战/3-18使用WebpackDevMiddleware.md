...
在 [1-6 使用 DevServer](../1入门/1-6使用DevServer.html) 中介绍过的 DevServer 是一个方便开发的小型 HTTP 服务器， DevServer 其实是基于 [webpack-dev-middleware](https://github.com/webpack/webpack-dev-middleware) 和 [Expressjs](https://expressjs.com) 实现的， 而 webpack-dev-middleware 其实是 Expressjs 的一个中间件。
...
从以上代码可以看出，从 `webpack-dev-middleware` 中导出的 `webpackMiddleware` 是一个函数，该函数需要接收一个 Compiler 实例。 在 [3-17 通过 Node.js API 启动 Webpack](3-17通过Node.jsAPI启动Webpack.html) 中曾介绍过 Webpack API 导出的 `webpack` 函数会返回一个Compiler 实例。
...
> 本实例<a href="../zip/3-18使用WebpackDevMiddleware.zip" target="_blank">提供项目完整代码</a>
